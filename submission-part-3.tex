...
\noindent
\todo{I think the fibrational setting is easier to describe. Maybe do that instead.}
...
\label{sec:extended-subobject-example}
...
\begin{diagram}
  ...
  \draw[fill=lightgray]
     (4.75, 2.25) -- (4.75, 1.75) -- (0.5, 1.75) -- (0.5, 1.5) -- 
     (0.25, 2) -- (0.5, 2.5) -- (0.5, 2.25) -- cycle;
  \node at (2, 2) {\textsf{pull back}};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Global ``truth values''}
...
In the rest of this section, we will describe some of these operations. For further details, see for instance \cite[pp. \todo{find the pages (chapter 6, I think)}]{Goldblatt:1984} or \cite[pp. 349--352]{Borceux:1994}.
...
The graph example from \cref{sec:extended-subobject-example} offers an example of a non-Boolean negation. There, the extension of $P$ is the sub-graph $G$ that is inserted into $F$. By contrast, the extension of $\lnot P$ is the graph consisting only of the point $q$. Note in particular that the edge $e_{2}$ is not part of the complement. This is due to the fact that $e_{2}$ begins inside the extension of $P$ but then leaves $P$ and ends up outside of $P$'s extension. When we take the complement of $P$, $e_{2}$ is therefore not part of the largest part of $F$ that is disjoint from $P$.
...
\noindent
\todo{I still think I should be doing this from the fibrational perspective. Quantifiers are way easier to understand that way. Can we get away with not discussing quantifiers at all?}
...
\noindent
\todo{define wholes in the internal language, and define the parthood relation in the internal language. Formulate some definitions like overlap.}
...
\noindent
\todo{prove some theorems like reflexivity, antisymmetry, and transitivity. discuss supplementation? extensionality? atoms?}
